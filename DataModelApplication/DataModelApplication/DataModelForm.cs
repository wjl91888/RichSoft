...
namespace DataModelApplication
{
        ...
        private void lbDatabaseObject_SelectedValueChanged(object sender, EventArgs e)
        {
            if (ValidateConnection(txtConnectionString.Text) == true)
            {
                ...
                if (rbTable.Checked == true)
                {
                    ...
                    if (CommonFileLibrary.IsExistFile(Directory.GetCurrentDirectory() + @"\" + lbDatabaseObject.SelectedValue + "Config" + ".xml") == true)
                    {
                        try
                        {
                            LoadConfigFile(Directory.GetCurrentDirectory() + @"\" + lbDatabaseObject.SelectedValue + "Config" + ".xml");
                        }
                        catch (Exception)
                        {
    ...
    GetDBDataTypeString((string)dgvrTemp.Cells["DBType"].Value);
                ...
                saveTableConfigFileDialog.InitialDirectory = Directory.GetCurrentDirectory();
        ...
        private void btnLoadTableConfig_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                openTableConfigFileDialog.InitialDirectory = Directory.GetCurrentDirectory();
        ...
        private void btnAutoGenerateCode_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (object objSelectedTable in lbDatabaseObject.SelectedItems)
                {
                    ...
                    xmlFileLibrary.FileName = Directory.GetCurrentDirectory() + @"\" + strTableName + "Config" + ".xml";
                    // 创建Xsl实例
                    XslCompiledTransform xslCompiledTransform = new XslCompiledTransform();
                    // 创建XmlDocment类的实例
                    XmlDocument xmlDocument = new XmlDocument();
                    // 把指定文件读入内存
                    xmlDocument.Load(strTableName + "Config" + ".xml");
                    // 创建代码文件目录
